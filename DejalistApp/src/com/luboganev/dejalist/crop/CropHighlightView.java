...
//    private static final int COLOR_OUTLINE_QUADRAT = 0xFFFF8A00; //orange
    private static final int COLOR_OUTLINE_QUADRAT = 0xFF33B5E5;
    private static final int COLOR_OUTLINE_CIRCLE = 0xFFEF04D6;
    private static final int COLOR_OUTLINE_NO_FOCUS = 0xFF000000;
    private static final float DEFAULT_MIN_CAP_WIDTH = 25F;
    ...
    protected void draw(Canvas canvas) {
        ...
        Path path = new Path();
        if (!hasFocus()) {
            mOutlinePaint.setColor(COLOR_OUTLINE_NO_FOCUS);
            ...
            if (mCircle) {
                ...
                path.addCircle(mDrawRect.left + (width  / 2),
                               ...
                               width / 2,
                               Path.Direction.CW);
                mOutlinePaint.setColor(COLOR_OUTLINE_CIRCLE);
            } else {
                path.addRect(new RectF(mDrawRect), Path.Direction.CW);
                mOutlinePaint.setColor(COLOR_OUTLINE_QUADRAT);
            }
    ...
    void growBy(float dx, float dy) {
        ...
        final float widthCap = mMinCapWidth;
        if (r.width() < widthCap) {
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle,
                      boolean maintainAspectRatio, float minCapWidth) {
        if (circle) {
        ...
        mCircle = circle;
        if(minCapWidth <= 0) mMinCapWidth = DEFAULT_MIN_CAP_WIDTH;
        else mMinCapWidth = minCapWidth;
